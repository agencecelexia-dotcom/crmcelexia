...
export function ProtectedRoute() {
  ...
  // Session exists but profile failed even after retry — probably a stale session
  // (user was deleted). Sign out and redirect to login.
  if (!profile) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4 text-center px-4">
        <p className="text-sm text-muted-foreground">
          Session expirée ou compte introuvable.
        </p>
        <button

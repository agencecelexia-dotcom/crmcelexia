                  ...
                  {comp != null && (
                    <span
                      ...
                      className={`inline-flex items-center rounded px-2 py-0.5 text-xs font-semibold ${
                        comp === 0 ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' :
                        comp === 1 ? 'bg-green-100 text-green-800' :
                        comp === 2 ? 'bg-amber-100 text-amber-800' :
                        'bg-red-100 text-red-800'
                      }`}
                    >
                      ⚔ {comp} concurrent{comp > 1 ? 's' : ''} LSA
                    </span>
                  )}
                  {slStatus && (
                    <span
                      ...
                      className={`inline-flex items-center rounded px-2 py-0.5 text-xs font-semibold ${
                        slStatus === 'replied' ? 'bg-purple-100 text-purple-800 border border-purple-300' :
                    ...
                    >
                      {slStatus === 'replied' ? '💬 A RÉPONDU' :
                       slStatus === 'opened' ? '👁 ouvert' :
                       slStatus === 'bounced' ? '⚠ bounce' :
                       slStatus === 'unsubscribed' ? '✋ unsub' :
                       '📧 envoyé'}
                      {slSentAt && ` · ${new Date(slSentAt).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' })}`}
                    </span>
                  )}
                </>
              )
